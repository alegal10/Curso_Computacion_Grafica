//Práctica 3                                    Galindo Granados Abner Alejandro
//Fecha de entrega: 21 febrero de 2026          320001567
#include<iostream>
...
int main() {
 ...
 //     
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Ejemplo 1.
		//view = glm::translate( view, glm::vec3( screenWidth / 2, screenHeight / 2,-900.0f ) ); // use with orthographic projection
		...
		model = glm::rotate(model, 45.0f, glm::vec3(0.1f, -5.0f, 0.0f)); // use to compare orthographic and perspective projection
		...
		//Tercer cubo
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-0.2f, 8.3f, 1.0f));
		model = glm::rotate(model, 2.0f, glm::vec3(0.0f, -50.0f, 3.0f)); // use to compare orthographic and perspective projection
		model = glm::scale(model, glm::vec3(3.0f, 2.5f, 4.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);

		//Cuarto Cubo
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(1.0f, 10.7f, 3.8f));
		model = glm::rotate(model, 2.7f, glm::vec3(0.0f,15.0f, 1.0f)); // use to compare orthographic and perspective projection
		...
		//Quinto Cubo
		model = glm::mat4(1);
		model = glm::translate(model, glm::vec3(-1.0f, 11.5f, 7.0f));
		model = glm::rotate(model, 5.3f, glm::vec3(10.0f, 0.0f, 0.0f)); // use to compare orthographic and perspective projection
