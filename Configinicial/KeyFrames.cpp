...
void interpolation(void)
{
	...
	//KeyFrame[playIndex].incFLegs = (KeyFrame[playIndex + 1].FLegs - KeyFrame[playIndex].FLegs) / i_max_steps;
...
int main()
{
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].head = 0; 
		KeyFrame[i].incHead = 0;
		KeyFrame[i].tail = 0;
		KeyFrame[i].incTail = 0;
		/*KeyFrame[i].FLegs = 0;
		KeyFrame[i].incFLegs = 0;*/
		KeyFrame[i].RLegs = 0;
		KeyFrame[i].incRLegs = 0;
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_R]) // Tecla R para subir
	{
		dogPosY += 0.001f;
	}
	if (keys[GLFW_KEY_F]) // Tecla F para bajar
	{
		dogPosY -= 0.001f;
	}
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			//FLegs += KeyFrame[playIndex].incFLegs;
